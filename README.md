...
Gắn khối lượng thông tin xác thực (GCP-ADC) vào bộ lập lịch luồng không khí và các container máy chủ web. Nó cũng gắn các thành phần cần thiết cho luồng không khí như logsthư mục src/config/variables.json và src/config/connections.json thiết lập Kết nối và Biến Airflow.

Gắn docker/airflow-entrypoint.sh vào bộ lập lịch Airflow và container máy chủ web, sau đó sử dụng nó làm tập lệnh điểm vào để nhập các tệp variables.json và connections.json đã gắn kết vào thời gian chạy khi khởi động lần đầu.
